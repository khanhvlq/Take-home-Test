...
from core.metadata_builder import PipelineMetadataBuilder
...
class SFTPSyncDagBuilder(BaseDagBuilder):
    def __init__(self, config: SFTPSyncDagConfig, connections: SFTPSyncConnectionConfig) -> None:
        ...
        self.assets = build_sftp_sync_assets(
            source_conn_type=self.connections.source_conn_type,
            source_conn_id=self.connections.source_conn_id,
            target_conn_type=self.connections.target_conn_type,
        ...
        metadata_builder = (
            PipelineMetadataBuilder(pipeline_id=self.sync_config.dag_id)
            .set_source(
                conn_type=self.connections.source_conn_type,
                conn_id=self.connections.source_conn_id,
                base_path=self.sync_config.source_base_path,
            )
            .set_target(
                conn_type=self.connections.target_conn_type,
                conn_id=self.connections.target_conn_id,
                base_path=self.sync_config.target_base_path,
            )
            .add_processing_component(
                name="checker",
                component_type="work-planner",
                config={"batch_size": self.sync_config.batch_size, "max_file_size_mb": self.sync_config.max_file_size_mb},
            )
            .add_processing_component(name="download", component_type="extractor")
        )

        for step in self.sync_config.transformations:
            metadata_builder.add_processing_component(
                name=step,
        ...
        self.catalog_metadata = (
            metadata_builder
            .add_processing_component(name="upload", component_type="loader")
            .add_processing_component(name="recheck", component_type="quality-gate")
            .set_orchestration(
                {
                    "schedule": self.sync_config.schedule,
                    "max_active_runs": self.sync_config.max_active_runs,
                    "max_active_tasks": self.sync_config.max_active_tasks,
    ...
    def register_tasks(self, dag: DAG) -> None:
        dag.doc_md = (
            ...
            "```json\n"
            f"{json.dumps(self.catalog_metadata.to_dict(), indent=2)}\n"
            "```"
        )
        ...
        checker_task = PythonOperator(
            ...
            op_kwargs={
                ...
                "max_file_size_mb": self.sync_config.max_file_size_mb,
                "transformations": list(self.sync_config.transformations),
            },
        ...
        download_task = PythonOperator.partial(
                task_id="download",
        ...
        upload_task = PythonOperator.partial(
                task_id="upload",
